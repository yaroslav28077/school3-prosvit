---
layout: layouts/page.njk
title: Публічна інформація
description: Офіційна інформація про заклад відповідно до законодавства України
permalink: /about/public-info/
blocks:
  - type: document_links
    category_title: 📋 Публічна інформація
    documents:
      - title: Статут закладу
        url: https://docs.google.com/document/d/1tdac-mg145bFm9oKOpPfa-1D7mJ0lw5N/edit?usp=sharing&ouid=105043052757299700735&rtpof=true&sd=true
        icon: "📜"
        description: Установчий документ закладу освіти

      - title: Ліцензія на провадження освітньої діяльності
        url: https://drive.google.com/file/d/1UdDCs6z-VgvSC0dqLF_A8ick_Sqp9QGg/view?usp=sharing
        icon: "🎓"
        description: Офіційна ліцензія МОН України

      - title: Територія обслуговування
        url: https://drive.google.com/drive/folders/11-ovMG6ZfanvkueaIpkcuSCWsEPcZyZJ?usp=sharing
        icon: "🗺️"
        description: Мікрорайон та територія закладу

      - title: Стратегія розвитку
        url: https://drive.google.com/drive/folders/1ekmSs25UJjB2h7KAj1rN6O5tOy486T9J?usp=sharing
        icon: "🚀"
        description: Стратегічні цілі та план розвитку

      - title: Освітня програма
        url: https://drive.google.com/file/d/1f_5dtzZRPbXzF6Z6VS3kI0DmjyzNyuab/view?usp=sharing
        icon: "📚"
        description: Основна освітня програма закладу

      - title: Мова освітнього процесу
        url: https://docs.google.com/document/d/1Kir4--YCk2WFk_8KEgpfidu7ArqUZm9FkRHSMGkCwSc/edit?usp=sharing
        icon: "🗣️"
        description: Інформація про мови навчання

      - title: Структура та органи управління закладу
        url: "#"
        icon: "🏛️"
        description: Організаційна структура (інформація готується)

      - title: Ліцензований обсяг та фактична кількість осіб
        url: https://drive.google.com/file/d/10atKNPViiQqSnHW5cXztR7Fr3CB5zudV/view?usp=sharing
        icon: "📈"
        description: Дані про контингент учнів

      - title: Умови доступності закладу освіти для навчання осіб з ООП
        url: https://drive.google.com/drive/folders/1i4jVOyxj_v14A0V5vLrYCn64aDAwuwr1?usp=sharing
        icon: "♿"
        description: Інклюзивність та доступність

      - title: Результати моніторингу якості освіти
        url: https://drive.google.com/file/d/1Yde8JT73eApzSvxU89t9KNB76Q8W0ozF/view?usp=sharing
        icon: "📉"
        description: Аналіз показників якості навчання

      - title: Річний звіт про діяльність закладу освіти
        url: https://drive.google.com/file/d/1u1gf_dz4nDhka0LYhb_HlyYo9heV4j2f/view?usp=sharing
        icon: "📝"
        description: Комплексний звіт про роботу за рік

      - title: Матеріально-технічне забезпечення закладу освіти
        url: https://drive.google.com/drive/folders/17PS-9-AtlifXfa3UujxTqX6_ORSdTLUf?usp=sharing
        icon: "🔧"
        description: Обладнання та матеріальна база

      - title: Штатний розпис
        url: https://drive.google.com/drive/folders/1dRW153XUu62yZZ82ZXHmqeBU2j8IBUNy?usp=sharing
        icon: "📊"
        description: Штатний розпис закладу

      - title: Кадровий склад закладу
        url: https://drive.google.com/file/d/1D3xZIW_ICD2DdmMW4trCfYjkqthz8F7V/view?usp=sharing
        icon: "👨‍🏫"
        description: Інформація про педагогічний колектив

      - title: Кошторис та фінансовий звіт про надходження та використання всіх отриманих коштів
        url: https://drive.google.com/drive/folders/1T0Ld0ML_VAJI7kt5oIsabpcpQxHqpyZJ?usp=sharing
        icon: "💰"
        description: Фінансова звітність та використання коштів

      - title: Наявність вакантних посад
        url: "#"
        icon: "💼"
        description: Інформація про вакансії (оновлюється)

      - title: Критерії оцінювання
        url: "#"
        icon: "✅"
        description: Інформація готується

      - title: Інша інформація що оприлюднюється за рішенням закладу освіти або на вимогу законодавства
        url: https://drive.google.com/drive/folders/1CispEDu53vqxhbltCAKEiBjeXr421NES?usp=sharing
        icon: "📁"
        description: Додаткові матеріали
---
